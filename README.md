...
# Практична робота 6
# Збір, обробка та інтелектуальний аналіз даних з сенсорів мобільного телефону
Цей проєкт аналізує рух на основі даних з акселерометра смартфона. За допомогою Python та бібліотек pandas, NumPy, matplotlib і SciPy виконується обчислення загального прискорення, фільтрація шуму та візуалізація даних у вигляді графіка.

Вимоги:
Для запуску цього проєкту потрібно встановити такі бібліотеки Python:


pip install pandas numpy matplotlib scipy
Як це працює:
Скрипт завантажує CSV-файл із даними прискорення по трьох осях (X, Y, Z), розраховує загальне векторне прискорення, згладжує дані за допомогою фільтра Савицького-Голея та будує графік зміни прискорення з часом.

Формула для обчислення загального прискорення:

​![image](https://github.com/user-attachments/assets/7ba4f804-a9fa-4fee-b1b1-d60ae02917fe)

  
де:
![image](https://github.com/user-attachments/assets/a8568fb9-40dc-4e76-a715-f7d6ce6bc284)



Основні компоненти:
Зчитування CSV-файлу з колонками:

Time_s — час у секундах,

Accel_X_m_s2, Accel_Y_m_s2, Accel_Z_m_s2 — компоненти прискорення.

Обчислення загального прискорення:


df['acc_total'] = np.sqrt(df['Accel_X_m_s2']**2 + df['Accel_Y_m_s2']**2 + df['Accel_Z_m_s2']**2)
Фільтрація шуму методом Савицького-Голея:


df['acc_filtered'] = savgol_filter(df['acc_total'], window_length=51, polyorder=3)
Побудова графіка для візуального аналізу:
...
Розрахунок середнього прискорення до та після фільтрації:


print("Середнє прискорення (сире):", df['acc_total'].mean())
print("Середнє прискорення (фільтроване):", df['acc_filtered'].mean())
Візуалізація:
Графік показує, як змінюється прискорення з часом, що дає змогу побачити характер руху та ефективність фільтрації шумів.

Приклад використання:
Замініть phone_accelerometer_data.csv на назву вашого CSV-файлу з даними.

Запустіть скрипт:

python acceleration_analysis.py
Скрипт виведе графік та середні значення прискорення.
![image](https://github.com/user-attachments/assets/9eb68b79-fb63-4cc9-a7c3-81a0f75cea1c)
![image](https://github.com/user-attachments/assets/53d5f574-bf16-4313-863f-9ca2bf7f8af8)



Висновок:
Цей підхід дозволяє ефективно аналізувати рух на основі даних з акселерометра, прибирати шуми за допомогою згладжування та отримувати ключові метрики, як-от середнє прискорення. Метод корисний для задач моніторингу активності, дослідження руху, мобільних додатків тощо.
